...
def create_sign_contract(public_key):
    verification_contract = "21"+public_key+"ac"
...
def construct_rawdata(tx_data):
    ...
    invoke_script = int_to_hex(len(signstr) / 2)+signstr
    verification_script = create_sign_contract(public_key)

    tx_data += "01"  # witness length
    tx_data += int_to_hex(len(invoke_script) / 2)
    tx_data += invoke_script
    tx_data += int_to_hex(len(verification_script) / 2)
    tx_data += verification_script
    raw_data = tx_data
    return raw_data
...
def transfer_tnc(address_from, address_to, value, contract_hash):
    op_data = construct_opdata(address_from=address_from, address_to=address_to, value=value,
...
if __name__ == "__main__":
    ...
    private_key = "f34f182b9f76187922af04edec05ddb173a536ec41eadf18d3e64ae7d4eb0f22"
    public_key = "0299dc85df93fee8ff2230af0418cf8c5000296f0aed514fcc0ab0dd969ce0bdb0"


    address_to = "AW547uHKTHEdHCWdJ9RWqxZsbB5T8eyUmY"
